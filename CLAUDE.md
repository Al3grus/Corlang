...
Current: v0.45.1 (versionCode 172). Live courses: `hr fr pt` — `de it es` are authored and still in the repo but HIDDEN (absent from `content/_index.json`) until the three focus courses are production-proven. Unhide = re-add the code to that manifest. See `MEMORY.md` (auto-memory) for the live resume point.
