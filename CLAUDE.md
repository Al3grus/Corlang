...
Current: v0.21.0 (versionCode 135). Live courses: `hr fr de it pt`. See `MEMORY.md` (auto-memory) for the live resume point.
...
```
Flavors (`distribution` dimension): `sideload` (default, in-app updater) and `play` (updater compiled out; AAB via `:app:bundlePlayRelease`). `versionCode`/`versionName` live in `app/build.gradle.kts` (currently 135 / 0.21.0). Room migrations required for any DB schema change (`data/db/AppDatabase`).
