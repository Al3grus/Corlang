...
Current: v0.47.2 (versionCode 176). Live courses: `hr pt` — `fr de it es` are authored and still in the repo but HIDDEN (absent from `content/_index.json`) until the live courses are production-proven. Unhide = re-add the code to that manifest. See `MEMORY.md` (auto-memory) for the live resume point.
