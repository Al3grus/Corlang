...
Current: v0.41.1 (versionCode 167). Live courses: `hr fr pt` — `de it es` are authored and still in the repo but HIDDEN (absent from `content/_index.json`) until the three focus courses are production-proven. Unhide = re-add the code to that manifest. See `MEMORY.md` (auto-memory) for the live resume point.
...
```
Flavors (`distribution` dimension): `sideload` (default, in-app updater) and `play` (updater compiled out; AAB via `:app:bundlePlayRelease`). `versionCode`/`versionName` live in `app/build.gradle.kts` (currently 167 / 0.41.1). Room migrations required for any DB schema change (`data/db/AppDatabase`).
