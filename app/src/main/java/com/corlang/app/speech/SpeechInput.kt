...
/**
 * One-shot Croatian speech recognition wrapper. Lets a learner say a phrase and get back what the
 * recognizer heard, so pronunciation can be checked against a target. Uses the system recognizer
 * (Google), which needs the RECORD_AUDIO permission and, usually, a network connection.
 *
...
class SpeechInput(private val context: Context) {
    ...
    /**
     * Starts listening for Croatian. [onListening] fires true when the mic opens and false when it
     * closes; [onResult] delivers the best transcript; [onError] delivers a human-readable message.
     */
    fun listen(
        ...
        val intent = Intent(RecognizerIntent.ACTION_RECOGNIZE_SPEECH).apply {
            ...
            putExtra("android.speech.extra.EXTRA_ADDITIONAL_LANGUAGES", arrayOf("hr-HR"))
        }
    ...
    private fun messageFor(error: Int): String = when (error) {
        ...
        SpeechRecognizer.ERROR_NETWORK, SpeechRecognizer.ERROR_NETWORK_TIMEOUT ->
            "Needs a connection for Croatian recognition."
        SpeechRecognizer.ERROR_INSUFFICIENT_PERMISSIONS -> "Microphone permission is needed."
        SpeechRecognizer.ERROR_LANGUAGE_NOT_SUPPORTED, SpeechRecognizer.ERROR_LANGUAGE_UNAVAILABLE ->
            "Croatian speech recognition isn't installed on this device."
