...
private fun FeynmanRunner(
    ...
    // Graded automatically from what was WRITTEN, not what the learner claims: derived from
    // the two saveable states above, so it survives rotation without its own persistence.
    // No self-tick (people tick what they meant), no AI-review button (retappable = token
    // drain); the offline rubric matcher grades instantly at reveal.
    val results = remember(concept.id, revealed, myExplanation) {
        if (!revealed) emptyList()
        else concept.rubricPoints.map { Grading.coversRubricPoint(it.point, myExplanation) }
    }
    ...
    Column(
        ...
        if (!revealed) {
            ...
            concept.rubricPoints.forEachIndexed { i, rp ->
                val got = results.getOrElse(i) { false }
                Surface(
            ...
            Row(modifier = Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                OutlinedButton(
                    onClick = {
                        // Loop: clear and try again, keeping the same concept. The graded
                        // results are derived state, so clearing the inputs resets them.
                        myExplanation = ""
                        revealed = false
                    },
